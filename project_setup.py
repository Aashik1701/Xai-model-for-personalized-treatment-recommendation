...
- NONINTERACTIVE: Set to 'true' to disable interactive prompts (useful for CI/CD)
- RECREATE_VENV: Set to 'true' to automatically recreate virtual environment
"""
...
def safe_input(prompt: str, default: str = "n") -> str:
    """
    Safe input function that respects non-interactive environments.

    Args:
        prompt: The prompt to display to the user
        default: Default value to return in non-interactive mode

    Returns:
        User input or default value in non-interactive mode
    """
    if os.getenv("NONINTERACTIVE", "").lower() in ("true", "1", "yes"):
...
class ProjectSetup:
    ...
    def create_virtual_environment(self):
        ...
        if self.venv_path.exists():
            ...
            # Check environment variable for automatic recreation
            recreate_env = os.getenv("RECREATE_VENV", "").lower() in (
                "true",
                "1",
                "yes",
            )
            ...
            if recreate_env:
                response = "y"
                print("   Auto-recreating due to RECREATE_VENV environment variable")
            else:
                response = safe_input("   Do you want to recreate it? (y/N): ", "n")
    ...
    def create_config_files(self):
        ...
        explainability_config = {
            "methods": ["shap", "lime"],
            "shap_config": {"explainer_type": "auto", "max_evals": 1000},
            "lime_config": {"num_features": 10, "num_samples": 1000},
            "attention_config": {"visualize_heads": True, "save_attention_maps": True},
        }
...
if __name__ == "__main__":
    # Check if this is being called by pip/build system
    is_build_process = (
        any(
            arg in " ".join(sys.argv)
            for arg in [
                "egg_info",
                "build",
                "install",
        ...
        or "pip" in str(sys.argv[0])
        or "pep517" in str(sys.modules.keys())
    )

    if is_build_process:
        print("📦 Build process detected - skipping interactive setup")
        # This allows the build system to complete without running our setup
        sys.exit(0)
    else:
        # Run normal interactive setup when called directly
        setup = ProjectSetup()
        setup.run_setup()
